...
def start_playwright(start_time):
    ...
    with conn.cursor() as cursor:
        ...
        print(start_time)
        if start_time[0] == "D" :
            command = "SELECT * FROM ci_staff_staff WHERE is_dayshift = True"
        elif start_time[0] == "N" :
            command = "SELECT * FROM ci_staff_staff WHERE is_dayshift = False"
        else :
            print("請輸入參數")
            return
        # 執行指令
        ...
        for row in result:
            ...
            if start_time=='N0640' and yesterday not in row[3] or start_time=='N1820' and today not in row[3] or start_time=='D0620' and today not in row[3]  or start_time=='D1840' and today not in row[3] :

                print(str(row[1])+"開始playwright")
                with sync_playwright() as p:
                  ...
                  page.locator( "xpath=//span[text()='自己的員工編號']/../../../following-sibling::div[1]/div/div[1]/div/div[1]/input").fill(str(row[1]))
                  if start_time=='N1820' or start_time=='D0620' :
                  ...
                  elif start_time=='N0640' or start_time=='D1840' :
                  ...
                  if start_time=='N1820'  or start_time=='N0640' or :
                    page.locator( "xpath=//div[@data-value='夜班']").click()
                  elif start_time=='D0620' or start_time=='D1840' :
                    page.locator( "xpath=//div[@data-value='日班']").click()
                  #page.locator("xpath=//span[text()='提交']/..").click()
                  pngfilename = now.strftime("%Y-%m-%d_%H:%M:%S")+'.png'
                  page.screenshot(path="/share/htdocs/"+row[1]+"/"+pngfilename)
                  if len(os.listdir("/share/htdocs/"+row[1]))>10:
                    os.remove("/share/htdocs/"+row[1]+"/"+sorted(os.listdir("/share/htdocs/"+row[1]))[0])
                    #print(os.listdir(row[0])[0])
            else:
                print(str(row[1])+"不開始playwright")
    cursor.close()
    conn.close()
...
def start_playwright_supervise(start_time):
    ...
    with conn.cursor() as cursor:
        # 查詢資料SQL語法最後一筆資料
        print(start_time)
        command = "SELECT * FROM cisupervise_supervise"
        # 執行指令
        cursor.execute(command)
        # 取得所有資料
        result = cursor.fetchall()
        for row in result:
            ...
            with sync_playwright() as p:
                ...
                page.locator( "xpath=//span[text()='員工編號']/../../../following-sibling::div[1]/div/div[1]/div/div[1]/input").fill(str(row[1]))
                ...
                page.locator("xpath=//span[text()='提交']/..").click()
                pngfilename = now.strftime("%Y-%m-%d_%H:%M:%S_")+row[1]+'.png'
